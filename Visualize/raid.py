...
diskArray = []
counter = 0
def emptyDictionary(numDisks):
    diskArray.clear()
    global counter
    counter = 0
    for x in range(numDisks):
        diskArray.append({})
...
class raid:
    ...
    def doSingleRead(self, disk, off, doNewline=False):
        ...
        if self.printPhysical:
            if(data1.level == 5):
                if(off == iter+1):
                    counter+=1
                    print((data1.numDisks-iter-1)%(data1.numDisks))
                    diskArray[(data1.numDisks-iter-1)%(data1.numDisks)].update({counter:int(0)})
                    iter+=1
            if self.printPhysical:
                print('  read  [disk %d, offset %d]  ' % (disk, off), end=' ')
                counter+=1
                diskArray[disk].update({counter:int(int(random.uniform(10000,20000)))})
            if doNewline:
                print('')
    ...
    def doSingleWrite(self, disk, off, doNewline=False):
        ...
        if self.printPhysical:
            ...
            counter+=1
            diskArray[disk].update({counter:int(off)});
...
def getDisks(data):
    ...
    if(data.level == 0 or data.level == 1):
        ...
        working = 'seq'
    if((data.level == 1) and data.writeFrac==0):
        fraction = 100
    else:
        fraction = data.writeFrac 
    
    if((data.level == 4 or data.level == 5) and data.writeFrac==100):
        fraction =  0

    

    parser.add_option('-s', '--seed',        default=0,      help='the random seed',                                action='store',       type='int',    dest='seed')
    parser.add_option('-D', '--numDisks',    default=data.numDisks,      help='number of disks in RAID',                        action='store',       type='int',    dest='numDisks') 
    parser.add_option('-C', '--chunkSize',   default='4k',   help='chunk size of the RAID',                         action='store',       type='string', dest='chunkSize') 
    parser.add_option('-n', '--numRequests', default=data.numRequest,     help='number of requests to simulate',                 action='store',       type='int',    dest='numRequests')
    parser.add_option('-S', '--reqSize',     default='4k',   help='size of requests',                               action='store',       type='string', dest='size')
    parser.add_option('-W', '--workload',    default=working, help='either "rand" or "seq" workloads',               action='store',       type='string', dest='workload')
    parser.add_option('-w', '--writeFrac',   default=fraction,      help='write fraction (100->all writes, 0->all reads)', action='store',       type='int',    dest='writeFrac')
    ...
    parser.add_option('-L', '--level',       default=data.level,      help='RAID level (0, 1, 4, 5)',                        action='store',       type='int',    dest='level')
    parser.add_option('-5', '--raid5',       default='LA',   help='RAID-5 left-symmetric "LS" or left-asym "LA"',   action='store',       type='string', dest='raid5type')
    parser.add_option('-r', '--reverse',     default=True,  help='instead of showing logical ops, show physical',  action='store_true',                 dest='reverse')
    ...
    print('')
    emptyDictionary(options.numDisks)
    ...
    if(data.level == 5):
        diskArray[(data1.numDisks-iter-1)%(data1.numDisks)].update({counter+1:int(0)})
    # process requests
    t = r.go()
